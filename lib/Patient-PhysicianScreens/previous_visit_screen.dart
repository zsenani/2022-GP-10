...
class PreviousVisitScreen extends StatefulWidget {
  PreviousVisitScreen(
    ...
    visitTime = visitT;
    visitId = visitID;
    age1 = DateTime.now().year - int.parse(patientDob.substring(0, 4));
    if (int.parse(patientDob.substring(5, 7)) > DateTime.now().month) {
      age1 = age1 - 1;
    } else if (int.parse(patientDob.substring(5, 7)) == DateTime.now().month) {
      if (int.parse(patientDob.substring(8, 10)) > DateTime.now().day) {
        age1 = age1 - 1;
      }
    }
  }
